...
if __name__ == '__main__':
    ...
    s.connect(('challenge.uiuc.tf', 11341))
    framerate = 44100
    tone_samples = int(0.3*framerate)
    ...
    for times in range(15):
        ...
        for x in range(727):
            data += recvuntil(s, "'")
        data = np.fromstring(zlib.decompress(base64.b64decode(data[2:-1])), dtype=np.float64)
        print data
        hexString = ""
        for i in range(0,11):
            goertzel = pygoertzel_dtmf(framerate)
            for j in data[i * tone_samples:(i + 1) * tone_samples]:
